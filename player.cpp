...
player_data new_player()
{
    ...
    double scr_width = screen_width() - screen_width() / 3.5;
    sprite_set_x(result.player_sprite, (scr_width - sprite_width(result.player_sprite)) / 2);
...
void update_player(player_data &player_to_update)
{
    ...
    double right_edge = left_edge + (screen_width() - screen_width() / 3.5) - 2 * SCREEN_BORDER;
    ...
    point_2d player_pos = sprite_position(player_to_update.player_sprite);
    if (player_pos.x < MIN - 1)
    {
        sprite_set_x(player_to_update.player_sprite, MIN);
        
    }
    if (player_pos.y < MIN - 1)
    {
        sprite_set_y(player_to_update.player_sprite, MIN);
    }
    if (player_pos.y > MAX + 1)
    {
        sprite_set_y(player_to_update.player_sprite, MAX);
    }

    if (player_pos.x > MAX + 1)
    {
    ...
    // prevents the fuel, potion, time and sheild from going below 5%
    if (player_to_update.fuel_pct >= 0.05)
    {
        player_to_update.fuel_pct -= 0.000278;
    }
    if (player_to_update.potion_pct >= 0.05)
    {
        player_to_update.potion_pct -= 0.000378;
    }

    if (player_to_update.time_pct >= 0.05)
    {
        player_to_update.time_pct -= 0.000178;
    }

    if (player_to_update.shield_pct >= 0.05)
    {
        player_to_update.shield_pct -= 0.000338;
    }
}
